...
**Validation:** `Pendo:updateOrchestrateEmailContent` tool description, parameter schema, and error messages.
...
2. `Pendo:getOrchestrateJourneySteps` — list Email steps; each step's `messageId` is the `emailId` on
   `Pendo:updateOrchestrateEmailContent`.
3. **Before drafting or saving** — for each email step you will update, call `Pendo:getOrchestrateEmail` with that
   `messageId` (for `emailType` and status). Follow `Pendo:updateOrchestrateEmailContent`'s `WithWorkflow` before calling
   the write tool.
4. Draft HTML; iterate with the user; call `Pendo:updateOrchestrateEmailContent` per approved email. After a successful
...
`{visitor.agent.field/}`, `{field|default/}`. Call `Pendo:visitorMetadataSchema` / `Pendo:accountMetadataSchema`
...
- Otherwise use `emailType` from `Pendo:getOrchestrateEmail` (step 3 above).
...
Uses plain greeting text — add single-brace personalization only after `Pendo:visitorMetadataSchema` confirms a
field exists.
...
**Save via MCP** (after `Pendo:getOrchestrateJourneySteps`): call `Pendo:updateOrchestrateEmailContent` using the tool's
