...
- Using `journeyId` or `stepId` as `emailId` (use `messageId` from `Pendo:getOrchestrateJourneySteps`).
- Reading `emailType` from `Pendo:getOrchestrateJourneySteps` (use `Pendo:getOrchestrateEmail` or what you set in
...
| Create timed out or transport error | Outcome unknown — journey may exist | `Pendo:listOrchestrateJourneys` with pagination; disambiguate by name; do not duplicate create |
...
| "Unknown template id" | Used UI template id from `Pendo:listOrchestrateJourneyTemplates` | Use built-in template create or `Pendo:createOrchestrateJourneyFromJson` — see create tool descriptions |
