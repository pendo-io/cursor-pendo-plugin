...
This skill uses **Pendo MCP tools** exclusively. Invoke them as **`Pendo:<toolName>`** in the tool list (e.g.
`Pendo:listAllApplications`, `Pendo:listOrchestrateJourneys`, `Pendo:getOrchestrateJourney`,
`Pendo:updateOrchestrateEmailContent`).

Read each tool's description, parameter schema, `WithWorkflow`, and `NotFor` before you call it. For
`subId`, `appId`, and journey resolution, follow `references/identify-journey.md`.

If Orchestrate tools are unavailable or auth fails:
...
2. **If no MCP tool** — you cannot see the Orchestrate UI. Do not assume the UI has the feature, and do not
   ...
   `Pendo:getOrchestrateJourney` / `Pendo:getOrchestrateJourneySteps`, or Pendo KB when you can. Otherwise ask the user
...
| Need | Read |
| ---- | ---- |
| Custom graph shape at create | `templateJson` JSON Schema on `Pendo:createOrchestrateJourneyFromJson`; sketches in `references/journey-graph-patterns.md` |
...
| **built-in template id**        | `mcpSingleEmailJourneyTemplate` → `Pendo:createOrchestrateJourneyFromTemplate`                     |
| **email step id**               | `messageId` from `Pendo:getOrchestrateJourneySteps` → `emailId` on `Pendo:updateOrchestrateEmailContent` |
